...
export class Utility {
	...
	static setupMeshProperties(child: any, isClient: boolean): void {
		if (child.type == 'Reflector') return
		...
		if (child.material !== undefined && child.material.map !== null) {
			...
			if (mat.map !== undefined && mat.map !== null) mat.map.anisotropy = 4
	...
	static getVehicle(world: WorldBase, name: string): Vehicle | null {
	...
	static getSeat(vehicle: Vehicle, userData: { [id: string]: any }): VehicleSeat | null {
		let seat: VehicleSeat | null = null
		vehicle.seats.forEach((st) => {
	...
	static getEntryPoint(vehicleSeat: VehicleSeat, userData: { [id: string]: any }): THREE.Object3D | null {
		let entryPoint: THREE.Object3D | null = null
		vehicleSeat.entryPoints.forEach((ep) => {
	...
	static defaultCamera() {
		let camera = new THREE.PerspectiveCamera(75, 1080 / 1920, 0.01, 20000)
	...
	static addText(obj: THREE.Object3D, message: string, color: THREE.Color | number = 0x006699) {
	...
	static addIndex(scene: THREE.Scene, mesh: THREE.Mesh | THREE.LineSegments) {
		...
		for (let i = 0; i < pos.count; i++) {
			...
			vertex.applyMatrix4(mesh.matrixWorld.clone())

			const obj = new THREE.Object3D()
			obj.position.copy(vertex.clone())
			obj.rotation.x = -Math.PI / 2
			Utility.addText(obj, i.toString(), (mesh.material as THREE.MeshBasicMaterial).color)
			scene.add(obj)
		}
	}
}
