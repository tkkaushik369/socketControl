...
export class Example extends BaseScene {
			...
			// world
			{
				// physics
				{
					...
					const cube = new THREE.Mesh(
						...
						new THREE.MeshStandardMaterial({ color: 0xaa00aa })
					)
					const cubePhy = new THREE.Mesh(
						new THREE.BoxGeometry(1, 1, 1),
						new THREE.MeshStandardMaterial({ color: 0xaa00aa })
					)
	...
	makeSeat(isDriver: boolean = false) {
		...
		const seat = new THREE.Mesh(
			...
			new THREE.MeshLambertMaterial({ color: isDriver ? 0xe5a00d : 0x00ffff })
		)
		seatGrp.add(seat)
		return seatGrp
	}
		...
		}
		{
			// chassy
			{
				const upper = new THREE.Mesh(
					...
					new THREE.MeshLambertMaterial({ color: 0x666666, transparent: true, opacity: 0.4 })
				)
	...
	makeHeli() {
		...
		}
		{
			...
			const rotor = new THREE.Mesh(
				new THREE.BoxGeometry(0.05, 2.4, 0.1),
				new THREE.MeshLambertMaterial({ color: 0xe5a00d })
			)
			...
			const rotorBack = new THREE.Mesh(
				new THREE.BoxGeometry(0.05, 0.5, 0.1),
				new THREE.MeshLambertMaterial({ color: 0xe5a00d })
			)
		...
		}
		{
			// landings
			{
				const bodyLanders = new THREE.Mesh(
					new THREE.BoxGeometry(1, 1, 1),
					new THREE.MeshLambertMaterial({ color: 0x666666, transparent: true, opacity: 0.8 })
				)
	...
	makeAirPlane() {
		const body = new THREE.Mesh()
		{
			// chassy
			const lower = new THREE.Mesh(
				new THREE.BoxGeometry(0.8, 0.5, 2.6),
				new THREE.MeshLambertMaterial({ color: 0x666666, transparent: true, opacity: 0.8 })
			)
			body.add(lower)
		...
		}
		{
			...
			const rotor = new THREE.Mesh(
				...
				new THREE.MeshLambertMaterial({ color: 0x33aaff })
			)
		...
		}
		{
			// "aileron",
			{
				...
				const aileron = new THREE.Mesh(
					new THREE.BoxGeometry(0.1, 1.2, 0.5),
					new THREE.MeshLambertMaterial({ color: 0xffaa77 })
				)
		...
		}
		{
			// collisions
			{
				const bodyColl = new THREE.Mesh(
					new THREE.BoxGeometry(1, 1, 1),
					new THREE.MeshLambertMaterial({ color: 0x666666, transparent: true, opacity: 0.8, wireframe: true })
				)
