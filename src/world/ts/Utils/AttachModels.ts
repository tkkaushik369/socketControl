	...
	public static makeCamera() {
		const group = new THREE.Group()
		{
			const camera = new THREE.Mesh(
				new THREE.BoxGeometry(1.6, 1, 0.4),
				new THREE.MeshBasicMaterial({ color: 0x888888, wireframe: false })
			)
			group.add(camera)

			const lens = new THREE.Mesh(
				new THREE.CylinderGeometry(0.4, 0.4, 0.2, 10, 1),
				new THREE.MeshBasicMaterial({ color: 0x2a2a2a, wireframe: false })
			)
	...
	public static makePointHighlight(scale: number = 1) {
		const group = new THREE.Group()
		{
			...
			const lineX = new THREE.LineSegments(
				edges,
				new THREE.LineDashedMaterial({ color: 0x00ffff, linewidth: 2, dashSize: 0.12, gapSize: 0.04 })
			)
			lineX.computeLineDistances()
			lineX.rotateY(Math.PI / 2)
			group.add(lineX)

			const lineY = new THREE.LineSegments(
				edges,
				new THREE.LineDashedMaterial({ color: 0xff00ff, linewidth: 2, dashSize: 0.12, gapSize: 0.04 })
			)
			lineY.computeLineDistances()
			lineY.rotateX(Math.PI / 2)
			group.add(lineY)

			const lineZ = new THREE.LineSegments(
				edges,
				new THREE.LineDashedMaterial({ color: 0xffff00, linewidth: 2, dashSize: 0.12, gapSize: 0.04 })
			)
