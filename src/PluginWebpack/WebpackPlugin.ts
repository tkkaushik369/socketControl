							...
							: [
									{
										...
										task: async (ctx: NativeDepsCtx) => {
											...
											for (const webpackNodeFile of webpackNodeFiles) {
												const hash = crypto.createHash('sha256')
												await pipeline(
													fs.createReadStream(path.resolve(firstArchDir, webpackNodeFile)),
													hash
												)
	...
	launchRendererDevServers = async (logger: Logger): Promise<void> => {
		...
		for (const entryConfig of configs) {
			...
			entryConfig.plugins.push(
				new ElectronForgeLoggingPlugin(logger.createTab(`Renderer Target Bundle (${entryConfig.target})`))
			)
