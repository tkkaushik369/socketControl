...
function rendererTargetToWebpackTarget(target: RendererTarget): WebpackTarget | WebpackTarget[] {
	switch (target) {
		...
		case RendererTarget.ElectronRendererNode:
			return [WebpackTarget.ElectronRendererNode, WebpackTarget.ElectronRenderer]
	}
}
...
export default class WebpackConfigGenerator {
	...
	async resolveConfig(config: Configuration | ConfigurationFactory | string): Promise<Configuration> {
		...
		return processConfig(this.preprocessConfig, rawConfig as Configuration | ConfigurationFactory)
	}
	...
	async getRendererConfig(rendererOptions: WebpackPluginRendererConfig): Promise<Configuration[]> {
		const entryPointsForTarget = {
			...
			electronRendererNode: [] as (WebpackPluginEntryPointLocalWindow | WebpackPluginEntryPoint)[],
		...
		const rendererConfigs = await Promise.all(
			[
				...
				await this.buildRendererConfigs(
					rendererOptions,
					entryPointsForTarget.electronRendererNode,
					RendererTarget.ElectronRendererNode
				),
				await this.buildRendererConfigs(
					rendererOptions,
		...
		return {
			...
			plugins: [
				new AssetRelocatorPatch(
					this.isProd,
					target === RendererTarget.ElectronRenderer || target === RendererTarget.ElectronPreload
				),
			],
		}
	}
	...
	async buildRendererConfigForWebOrRendererTarget(
		...
		target: RendererTarget.Web | RendererTarget.ElectronRenderer | RendererTarget.ElectronRendererNode
		...
		if (target === RendererTarget.ElectronRendererNode)
	...
	async buildRendererConfigs(
		...
		if (
			...
			target === RendererTarget.ElectronRendererNode
		) {
			...
			entryPoints.forEach((entryPoint) =>
				(entryPoint.preload.config ? entryPointsWithPreloadConfig : entryPointsWithoutPreloadConfig).push(
					entryPoint
				)
			)
			...
			entryPointsWithPreloadConfig.forEach((entryPoint) => {
				rendererConfigs.push(
					this.buildRendererConfigForPreloadOrSandboxedPreloadTarget(rendererOptions, [entryPoint], target)
				)
			})
			return rendererConfigs
		} else {
