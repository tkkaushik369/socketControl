...
export class CannonDebugRenderer {
	...
	constructor(scene: THREE.Scene, world: CANNON.World, options?: object) {
		...
		this._allMeshMaterials.push(this._sphereMaterial)
		...
		this._allMeshMaterials.push(this._boxMaterial)
	...
	private _createMesh(shape: CANNON.Shape): THREE.Mesh | THREE.Points {
		...
		switch (shape.type) {
			case CANNON.Shape.types.SPHERE:
				mesh = new THREE.Mesh(this._sphereGeometry, this._sphereMaterial)
				break

			case CANNON.Shape.types.BOX:
				mesh = new THREE.Mesh(this._boxGeometry, this._boxMaterial)
				break
			...
			case CANNON.Shape.types.PLANE:
				mesh = new THREE.Mesh(this._planeGeometry, this._boxMaterial)
				break
	...
	public setWireframe(isSet: boolean) {
		this._allMeshMaterials.forEach((material) => {
			material.wireframe = isSet
		})
	}

	public setEdges(isSet: boolean) {
		this.showEdges = isSet
	...
	public setOpacity(value: number) {
		this.opacity = value
		this._allMeshMaterials.forEach((material) => {
			material.opacity = this.showEdges ? 0 : value
		})
		this._meshes.forEach((mesh) => {
