...
export class Noise {
	...
	constructor() {
		...
		this.perlin2 = this.perlin2.bind(this)
		this.perlin3 = this.perlin3.bind(this)
	...
	// 3D simplex noise
	private simplex3(xin: number, yin: number, zin: number) {
	...
	// 2D Perlin Noise
	private perlin2(x: number, y: number) {
	...
	// 3D Perlin Noise
	private perlin3(x: number, y: number, z: number) {
		// Find unit grid cell containing point
		var X = Math.floor(x),
			Y = Math.floor(y),
			Z = Math.floor(z)
